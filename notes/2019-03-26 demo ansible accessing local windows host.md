## 2019-03-26 Demo Ansible accessing local Windows host
...
- Activated the ansible virtual environment and installed the pywinrm package:
    ...
    ```
- Added the file /etc/ansible/hosts with a single line in WSL Debian environment (password redacted in notes) specifying that a WinRM connection to port 5986 (the WinRM port)  be used:

    ```console
    root@circe-win10:/etc/ansible# cat hosts
    [windows]
    localhost ansible_port=5986 ansible_connection=winrm ansible_user=tmcphill ansible_password=XXXXXXXX ansible_winrm_server_cert_validation=ignore
    ```
